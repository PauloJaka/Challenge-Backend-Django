...
def test_create_wallet_with_zero_balance():
    ...
    assert before_execution <= saved_wallet.created_at <= after_execution, "Data de criação fora do intervalo esperado"
    assert saved_wallet.last_transaction_date == saved_wallet.created_at, "Data da última transação deve ser igual à data de criação"

    repo_mock.save.assert_called_once(), "Repositório deve ser chamado uma vez"
    assert returned_wallet == repo_mock.save.return_value, "Carteira retornada deve ser a mesma que foi salva"
...
def test_create_wallet_with_negative_balance_raises_error():
    ...
    # Assert
    assert str(exc_info.value) == "Saldo inicial não pode ser negativo", "Mensagem de erro incorreta"
