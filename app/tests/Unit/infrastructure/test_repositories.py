...
class TestDjangoUserRepository(TestCase):
    ...
    def test_save_user_with_timezone(self):
        # Arrange
        user = User(
            id=None, name="Bob", cpf="98765432100", password="senha123", created_at=None
        )
...
def test_get_wallet():
    ...
    assert core_wallet.last_transaction_date == timezone.localtime(
...
def test_save_wallet():
    ...
    assert saved_wallet.last_transaction_date == timezone.localtime(
        django_wallet.last_transaction_date
    )
...
def test_save_wallet_update():
    ...
    assert saved_wallet.last_transaction_date == timezone.localtime(
        updated_wallet.last_transaction_date
    )
